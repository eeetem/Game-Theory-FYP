...
//
	...
	private static Vector2 GetMovementDirection()
	{
		...
		if (state.IsKeyDown(Keys.S)) movementDirection += Vector2.UnitY;
		if (state.IsKeyDown(Keys.W)) movementDirection -= Vector2.UnitY;
		if (state.IsKeyDown(Keys.A)) movementDirection -= Vector2.UnitX;
		if (state.IsKeyDown(Keys.D)) movementDirection += Vector2.UnitX;
		...
		if (ForceMoving)
		{
			var difference = MoveTarget - Cam.Position;
	...
	public static void Update(GameTime gameTime)
	{
		...
		if (Keyboard.GetState().IsKeyDown(Keys.OemPlus))
			diff = 25;
		else if (Keyboard.GetState().IsKeyDown(Keys.OemMinus)) diff = -25;
